...
class JsonInfo():
    def __init__(self, url):
        self.info = json.loads(getURLContent(url).decode())
        if 'code' in self.info and self.info['code'] != 0:
            if 'message' in self.info:
                print("【Error】code=%d, msg=%s, url=%s" %
                      (self.info['code'], self.Getvalue('message'), url))
                self.ERROR_MSG = self.Getvalue('message')
            elif 'error' in self.info:
                print("【Error】code=%d, msg=%s, url=%s" %
                      (self.info['code'], self.Getvalue('error'), url))
...
def ParseComment(danmu):
    ...
    for i, comment in enumerate(comment_element):
        ...
        danmu.t_stamp = time.strftime('%Y-%m-%d %H:%M:%S',
                                      time.localtime(float(p[4])))
        danmu.mid_crc = p[6]
        danmu.danmu_color = ConvertColor(int(p[3]))
        danmu_fontsize = int(p[2])
        if len(comment.childNodes) != 0:
            danmu.content = str(comment.childNodes[0].wholeText).replace(
                '/n', '\n')
        else:
            danmu.content = ""
        yield danmu